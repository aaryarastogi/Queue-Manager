...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8000/average-wait-time/${queueId}`, authHeader)
      .then(res => setAverageWait(res.data.averageWaitTime || 0));

    axios.get(`http://localhost:8000/queue-trends/${queueId}`, authHeader)
      .then(res => setQueueTrends(res.data));

    axios.get(`http://localhost:8000/current-length/${queueId}`, authHeader)
    ...
    <div className="min-h-screen bg-[#f8fafb] p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div
              className="bg-white rounded-xl p-6 shadow-lg border border-[#e5e7eb] hover:shadow-xl transition-all duration-200"
            >
